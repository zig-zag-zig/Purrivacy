...
describe('safeStringify', () => {
    ...
    it('falls back gracefully when redact hits circular reference in write path', () => {
        // The logger's redact function recurses on objects without cycle detection.
        // safeStringify has a try/catch that catches the resulting RangeError and
        // emits a fallback error log entry. This test verifies that path.
        jest.resetModules();
        process.env.LOG_LEVEL = 'debug';
        ...
        // logger.info calls redact which hits the circular ref, causing a stack overflow.
        // The stack overflow bubbles up from redact (before safeStringify).
        // This is a known limitation — verify it throws rather than silently corrupting.
        expect(() => {
